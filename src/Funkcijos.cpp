...
void generuotiFaila(int nd_count, long long kiek) {
    ...
    std::cout << "Sugeneruotas failas studentai_" << kiek << ".txt su "
              << kiek << " studentais ir " << nd_count << " ND.\n";
}
...
std::vector<Studentas> nuskaitytiIsFailo(const std::string& failoPavadinimas) {
    ...
    std::string header;
    std::getline(in, header); // praleidziam antraste
...
void isvestiRezultatus(const std::vector<Studentas>& studentai,
    ...
    if (!out) {
        std::cerr << "Nepavyko sukurti failo: " << failoPavadinimas
                  << std::endl;
        return;
    }
...
void testavimas() {
    ...
    for (auto n : sizes) {
        ...
        auto start_nuskaitymas =
            std::chrono::high_resolution_clock::now();
        test_grupe = nuskaitytiIsFailo(fail_name);
        auto end_nuskaitymas = std::chrono::high_resolution_clock::now();
        double laikas_nuskaitymas =
            std::chrono::duration<double>(end_nuskaitymas - start_nuskaitymas)
                .count();

        if (test_grupe.empty()) {
            std::cout << "Failas " << fail_name
                      << " tuscias arba nepavyko nuskaityti.\n";
            continue;
        }

        auto start_rikiavimas =
            std::chrono::high_resolution_clock::now();
        ...
        double laikas_rikiavimas =
            std::chrono::duration<double>(end_rikiavimas - start_rikiavimas)
                .count();

        auto start_padalijimas =
            std::chrono::high_resolution_clock::now();
        StudentGroups groups = suskirstytiStudentus(test_grupe);
        auto end_padalijimas = std::chrono::high_resolution_clock::now();
        double laikas_padalijimas =
            std::chrono::duration<double>(end_padalijimas - start_padalijimas)
                .count();

        auto start_irasymas =
            std::chrono::high_resolution_clock::now();
        isvestiRezultatus(groups.kietiakiai,
                          "kietiakiai_" + std::to_string(n) + ".txt");
        isvestiRezultatus(groups.vargsiukai,
                          "vargsiukai_" + std::to_string(n) + ".txt");
        ...
        double laikas_total = laikas_nuskaitymas + laikas_rikiavimas +
                              laikas_padalijimas + laikas_irasymas;

        std::cout << "Failo nuskaitymo laikas: " << laikas_nuskaitymas << " s\n";
        std::cout << "Rikiavimo laikas: " << laikas_rikiavimas << " s\n";
        std::cout << "Padalijimo i vargsiukus ir kietakius laikas: "
                  << laikas_padalijimas << " s\n";
        std::cout << "Irasymo laikas i failus laikas: " << laikas_irasymas
                  << " s\n";
